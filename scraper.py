...
Once the flask instance is running, test connectivity from a separate terminal window with the command:
    $ curl -u user1:pass1 http://127.0.0.1:3000/Test/get_scraper_resource
...
Users = {
    'admin':'secret', # This is required for our current solution because we're directly accessing this flask
...
def addUser():
    ...
    if args['new_user'] in Users:
        ...
        return make_response(jsonify({'Result': 'Request Failed: Already taken'}))
    else:
...
def weather(city_name):
    ...
    return jsonify({'Weather Report\nlocation': city_name},
                   {'temperature': str(mainText['temp'])},
                   {'pressure': str(mainText['pressure'])},
                   {'humidity': str(mainText['humidity'])})
# end of function
...
def covid(state_name):
    ...
    return jsonify({'State': str(state_R.text).strip()},
                   {'Total Cases': str(totalCases_R.text).strip()},
                   {'Total Dead': str(totalDeaths_R.text).strip()},
                   {'Total Recovered': str(totalRec_R.text).strip()})
# end of function
